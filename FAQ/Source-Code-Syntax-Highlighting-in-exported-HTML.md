...
For example a block of HTML code like this:

![](../images/MarkdownCodeBlock.png)

is turned into HTML like this by the Markdown parser:

![](../images/MarkdownCodeBlock-rendered.png)

HighlightJS then turn this HTML into nicely syntax colored text. There are a number of other JavaScript based Syntax Highlighters available, but highlightJs is amongst the easiest and fastest rendering ones so that's what we provide.

Note that code is HTML encoded so it's safe to display without script injection, but the actual code is not in any way marked up for syntax coloring other than using:
...
```
to identify the code as a code block. There are many JavaScript based highlighting tools available that can understand this syntax, and those that don't are usually easy to program to provide the required formatting.
